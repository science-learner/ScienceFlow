...
"""Pure event-to-state projections for the legacy-compatible LHR wire format."""
...
from scienceflow.research.solver.lnr.lifecycle.stage.metrics.score_summary import score_record_from_stage_payload
...
def _atomic_write_json(path: Path, payload: dict[str, Any]) -> None:
    JsonStateProjection(path).write(payload)


def _atomic_write_jsonl(path: Path, events: list[dict[str, Any]]) -> None:
    JsonEventJournal(path).rewrite(events)


def _read_events(path: Path) -> list[dict[str, Any]]:
    return JsonEventJournal(path).read()
