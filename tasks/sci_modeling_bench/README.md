...
`N` is the submitted batch size and `K` is the configured summary size used by
K-based metrics and diagnostics. Query limits below are per worker when using
the included SciModelingBench profile.
